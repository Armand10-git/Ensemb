...
| T02 | `TenancyModule` : middleware de résolution de tenant par sous-domaine, contexte de requête (`AsyncLocalStorage`), extension Prisma d'auto-scoping par `organizationId` | Une requête sur un sous-domaine ne retourne jamais les données d'un autre tenant (test d'isolation) | ✅ 2026-07-19 — `TenantContextService` (AsyncLocalStorage, lève InternalServerErrorException hors contexte), `TenancyMiddleware` (lookup Redis TTL 1h + fallback Prisma, 404 neutre), extension `buildTenantExtension` (auto-scope findMany/findFirst/findUnique sur User/Role/AuditLog), `GET /api/v1/public/organizations/by-subdomain/:subdomain` (mobile). Routes exemptes : /health, /ready, /api/v1/auth/*, /api/v1/public/*. 11 tests unitaires verts (isolation inter-tenant, hors-contexte, cache, 404). Build TypeScript strict vert. |
