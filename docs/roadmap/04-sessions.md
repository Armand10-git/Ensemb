...
| T06 | `Plan` + `Subscription` + `PlatformSetting` (§4), seed des plans par défaut (avec `trialDurationDays`/`trialRevenueCapAmount`), écran admin pour configurer la fenêtre de lancement, `QuotaGuard` sur les endpoints de création | Dépasser un quota renvoie une erreur explicite, pas une erreur 500 ; la fenêtre de lancement est modifiable sans déploiement (§17 point R) | ✅ 2026-07-19 — Migration `add_plan_subscription_platform_setting` : modèles `Plan` (Decimal, maxUsers/maxWarehouses/maxProducts nullable), `Subscription` (enum `SubscriptionStatus`, `@unique organizationId`), `PlatformSetting` (clé/valeur JSON). Seed idempotent : 3 plans (starter 5k/50k XAF, pro 15k/150k, enterprise 40k/400k), `PlatformSetting.launchPromoEndsAt = "2026-09-30T23:59:59Z"`. `RegistrationService` : `computeTrialEndsAt()` → `computeTrialPeriod(now, launchPromoEndsAt, trialDurationDays)` (pure, testable) ; lecture `PlatformSetting` + plan starter dans la transaction → `trialEndsAt = launchPromoEndsAt` (fenêtre) ou `now + trialDurationDays` (post-fenêtre) ; `Subscription TRIALING` créée dans la même transaction atomique. `BillingModule` : `BillingService.getSubscription()` (NotFoundException si absente) + `getPlatformSetting()` (désérialisation JSON, InternalServerErrorException si invalide) ; `QuotaGuard` + `@CheckQuota('users'|'warehouses'|'products')` : 403 explicite en français avec nom du plan et limite, 500 générique sur erreur interne. Importé dans `AppModule`. 142 tests unitaires verts (computeTrialPeriod ×4, BillingService ×3, QuotaGuard ×7, registration mis à jour ×14+). 22 tests e2e verts (billing ×6 — fenêtre/post-fenêtre/quota ok/403/illimité/401 ; registration ×9 mis à jour avec cleanup Subscription). Lint + typecheck stricts verts. Dette : `@CheckQuota('users')` sur `POST /api/v1/users` à apposer quand `UsersModule` sera créé (Bloc C). |
