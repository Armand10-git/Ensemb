...
> **Dette e2e ouverte par le fix S18 (`9ec86db`) — résolue pendant S19.** `JwtAuthGuard` vérifie désormais que le tenant résolu par `TenancyMiddleware` correspond à l'organisation du token (anti-IDOR inter-tenant), ce qui le rend dépendant de `TenantContextService`. Trois conséquences détectées et corrigées pendant S19 :
> 1. ✅ `TenancyMiddleware.extractSubdomain` traitait une adresse IPv4 littérale (ex. `127.0.0.1`, hôte par défaut de `supertest`) comme un sous-domaine (`"127"` matche `SUBDOMAIN_REGEX`), court-circuitant le fallback `X-Organization-Id` prévu pour le dev/CLI/tests. Garde IPv4 ajoutée + mock `tenancy.middleware.spec.ts` corrigé (`headers: {}` manquant).
> 2. ✅ 16 des 20 suites e2e (`transfer`, `adjustment`, `product`, `catalog`, `partners`, `warehouse`, `unit`, `product-warehouse`, `notification`, `uploads`, `branding`, `backup`, `billing`, `billing-webhook`, `encryption`, `auth`) n'importaient pas `TenancyModule` dans leur `Test.createTestingModule` — chaque requête échouait en 500 (`Cannot read properties of undefined (reading 'tryGetOrganizationId')`). Corrigé dans les 16 fichiers (import `TenancyModule` + header `X-Organization-Id` sur chaque requête authentifiée, org du token pour les tests d'isolation inter-tenant — jamais l'org de la ressource ciblée). `auth.e2e.spec.ts` déplacé de `src/modules/auth/test/` vers `apps/api/test/` au passage (seul spec e2e hors de `apps/api/test/`, cause d'une violation `rootDir` au typecheck). `pnpm test:e2e` est de nouveau fiable sur l'ensemble du projet (20/20 suites, 194/194 tests).
> 3. ✅ Régression de production distincte, découverte en creusant la cause n°2 : une fois `TenancyModule` importé, son middleware (`forRoutes('*')`) interceptait aussi `/api/v1/webhooks/*` — absent de `EXEMPT_ROUTES`. `POST /api/v1/webhooks/billing` (callback `PAYMENT_AGGREGATOR_CALLBACK_URL`, sans sous-domaine tenant) aurait donc échoué en 404 en production, cassant la confirmation de paiement d'abonnement SaaS, indépendamment de tout test. `/api/v1/webhooks/(.*)` ajouté à `EXEMPT_ROUTES` (`tenancy.module.ts` + défense secondaire `tenancy.middleware.ts`) — sécurité de ces routes assurée par la signature de l'agrégateur et l'idempotence `WebhookEvent` (§17 point V), pas par la résolution tenant ; vérifié qu'aucun code de `WebhookController`/`BillingService` ne dépend de `TenantContextService`.

> **Design system frontend introduit rétroactivement (post-S19) — ne pas revenir aux styles inline.** Jusqu'ici, chaque écran (S09→S19) utilisait des styles inline bruts (`style={{...}}`) faute de composants partagés — `packages/ui` était un stub vide et `tailwind.config.ts` n'avait aucun thème, malgré la stack annoncée (React + shadcn/ui + Tailwind). Un vrai design system a été construit dans `apps/web` (pas `packages/ui` — les primitives shadcn/ui reposent sur Radix UI/le DOM, non partageables avec `apps/mobile` en React Native ; `packages/ui` reste un stub) :
> - `apps/web/tailwind.config.ts` : couleurs `brand-{50..950}` (vert, `docs/ux/tokens.md`), `amber`/`danger`/`info` sémantiques ; polices `font-display` (Bricolage Grotesque, titres), `font-sans` (Instrument Sans, corps), `font-mono` (IBM Plex Mono, montants/références — classe utilitaire `.tabular`) ; `rounded-field`/`rounded-card` ; `shadow-1`/`shadow-2`.
> - `apps/web/src/components/ui/` : primitives shadcn/ui (`button`, `input`, `label`, `textarea`, `card`, `badge`, `table`, `sheet`, `alert-dialog`, `tooltip`, `dropdown-menu`, `skeleton`, `sonner`) + `native-select.tsx` (un `<select>` natif stylé, **volontairement pas Radix Select** — les tests utilisent `userEvent.selectOptions()`, incompatible avec un listbox custom).
> - `apps/web/src/components/page-states.tsx` : `PageHeader`/`TableSkeleton`/`EmptyState`/`ErrorState` — grammaire des 5 états (standards.md règle 2).
> - `toast` de `sonner` remplace tout composant `ToastList` maison.
> - Les 11 écrans existants (Ventes, Ajustements, Transferts, Produits, Clients, Fournisseurs, Marques, Catégories, Unités, Devises, Entrepôts) + `AppLayout`/`login` ont été réalignés sur ce système, logique métier strictement inchangée (mêmes hooks React Query, mêmes endpoints, mêmes payloads — seule la présentation a changé).
> **Pour toute nouvelle session frontend** : réutiliser ces primitives, ne jamais réintroduire de styles inline ni de composants Sheet/AlertDialog/Toast/Skeleton maison. La théming dynamique par tenant (couleur de marque du tenant en variables CSS, standards.md règle 9) n'est pas câblée — structure de tokens compatible, mais implémentation reportée à une session dédiée.
