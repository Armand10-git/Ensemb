...
function makeJob(data: Partial<PdfJobData>, name = 'purchase.generatePdf') {
...
describe('PurchasePdfWorker', () => {
  ...
  let worker: PurchasePdfWorker;

  beforeEach(() => {
    purchaseService = { findOne: jest.fn() };
    pdfService = { render: jest.fn().mockResolvedValue(Buffer.from('%PDF-1.4')) };
    storageService = {
      upload: jest.fn().mockResolvedValue(`${ORG_ID}/pdf/purchase/${PURCHASE_ID}.pdf`),
  ...
  it('nom de job inconnu → ignoré, aucun service appelé', async () => {
    await worker.process(
      makeJob({ organizationId: ORG_ID, documentId: PURCHASE_ID }, 'other.job'),
    );
  ...
  it('succès : recharge l\'achat, uploade le PDF à la bonne clé et émet pdf:ready', async () => {
    purchaseService.findOne.mockResolvedValue(FAKE_PURCHASE);

    await worker.process(
      makeJob({ organizationId: ORG_ID, documentId: PURCHASE_ID, requestedBy: 'user-1' }),
    );
    ...
    expect(storageService.upload).toHaveBeenCalledWith(
      `${ORG_ID}/pdf/purchase/${PURCHASE_ID}.pdf`,
      expect.any(Buffer),
      'application/pdf',
    );
  ...
  it('achat introuvable (NotFoundException) → no-op, pas de relance, pas d\'événement d\'échec', async () => {
    purchaseService.findOne.mockRejectedValue(new NotFoundException('Achat introuvable.'));

    await expect(
      worker.process(makeJob({ organizationId: ORG_ID, documentId: PURCHASE_ID })),
