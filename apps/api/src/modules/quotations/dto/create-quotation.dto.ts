...
/** Ligne d'un devis (S28). Mirror exact de SaleDetailSchema, quoteUnitId au lieu de saleUnitId. */
export const QuotationDetailSchema = z.object({
  /** UUID du produit du devis. Vérifié côté service (ownership org). */
  ...
  /** UUID de l'unité choisie pour cette ligne — optionnel. */
  quoteUnitId: z.string().uuid('quoteUnitId doit être un UUID valide').optional(),

  /** Prix unitaire — string décimale, non négatif. */
  price: nonNegativeDecimalString,

  /** Quantité — string décimale convertie en Decimal dans le service. Toujours > 0. */
...
export const CreateQuotationSchema = z.object({
  ...
  /** Lignes du devis — au moins une ligne requise. */
  details: z
