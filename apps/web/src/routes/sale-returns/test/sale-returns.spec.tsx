...
import SaleReturnsPage from '../index';
...
/** Retour tel que renvoyé par GET /sale-returns/:id (avec relations sale/warehouse/details). */
function makeSaleReturnDetail(overrides: Record<string, unknown> = {}) {
  return {
    id: SALE_RETURN_ID,
    reference: 'RVT-2026-000001',
    ...
    sale: { id: SALE_ID, reference: 'VTE-2026-000001' },
...
/** Ligne telle que renvoyée par GET /sale-returns (liste paginée — pas de relations, cf. service). */
function makeSaleReturnListItem(overrides: Record<string, unknown> = {}) {
  const full = makeSaleReturnDetail() as Record<string, unknown>;
  delete full.sale;
...
const listResp        = { data: [makeSaleReturnListItem()], total: 1, page: 1, limit: 20 };
...
function mockGet(detail: ReturnType<typeof makeSaleReturnDetail>, payments: unknown[] = []) {
  mockApi.get.mockImplementation((path: string) => {
    ...
    if (path.includes('/sale-returns?'))     return Promise.resolve(listResp);
    if (path.includes('/sale-returns/'))     return Promise.resolve(detail);
...
async function openDetail() {
  renderPage();
  await waitFor(() => screen.getByText('RVT-2026-000001'));
  await userEvent.click(screen.getByText('RVT-2026-000001'));
...
describe('SaleReturnsPage', () => {
  ...
  it("affiche l'état vide si aucun retour", async () => {
    ...
    renderPage();
    await waitFor(() => expect(screen.getByText('Aucun retour de vente')).toBeInTheDocument());
  });
  ...
  it('affiche la liste des retours avec référence et total', async () => {
    renderPage();
    await waitFor(() => expect(screen.getByText('RVT-2026-000001')).toBeInTheDocument());
  ...
  it('le bouton "Valider le retour" déclenche PATCH .../validate et affiche un toast de succès', async () => {
    mockApi.patch.mockResolvedValue(makeSaleReturnDetail({ status: 'COMPLETED' }));
    ...
    expect(mockApi.patch).toHaveBeenCalledWith(`/sale-returns/${SALE_RETURN_ID}/validate`, {});
  });

  it('le bouton "Valider le retour" est absent quand status === COMPLETED', async () => {
    mockGet(makeSaleReturnDetail({ status: 'COMPLETED', paymentStatus: 'UNPAID' }));
    await openDetail();
  ...
  it('ajoute un remboursement : ouvre le Sheet, remplit le formulaire, POST sur la bonne route, toast de succès', async () => {
    ...
    expect(mockApi.post).toHaveBeenCalledWith(
      `/sale-returns/${SALE_RETURN_ID}/payments`,
  ...
  it('ouvre automatiquement le détail via le deep-link ?open=<id>', async () => {
    window.history.pushState({}, '', `/sale-returns?open=${SALE_RETURN_ID}`);

    renderPage();

    await waitFor(() => expect(screen.getByText(/Retour RVT-2026-000001/)).toBeInTheDocument());
    ...
    window.history.pushState({}, '', '/sale-returns');
  });
});
