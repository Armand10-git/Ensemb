  ...
  'fixed z-50 flex flex-col gap-0 bg-white shadow-2 transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300',
  {
    variants: {
      side: {
        ...
        left:
          'inset-y-0 left-0 h-full w-full max-w-[280px] border-r border-neutral-200 data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left',
      },
    },
...
interface SheetContentProps
  ...
  /** Masque le bouton de fermeture (X) — pour un état bloquant où la fermeture doit passer
   * uniquement par une action explicite du contenu (ex. attente d'un paiement en cours). */
  hideClose?: boolean;
}
